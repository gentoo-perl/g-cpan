...
sub generatePackageInfo
{
    ...
    unless (defined($gcpan_run->{portage}{lc($ebuild_wanted)}{found}))
    {
        ...
        # but with one exception 'Module::Build' due to perl-module.eclass requirements
        if ( $ebuild_wanted ne 'Module::Build' ) {
    ...
    {
        return if $original_ebuild eq 'perl';
    ...
    if (!defined($gcpan_run->{portage}{lc($ebuild_wanted)}{found}))
    {
            ...
            && lc($gcpan_run->{cpan}{lc($original_ebuild)}{portage_name}) ne 'perl')
        {
            ...
            foreach my $dep (keys %{$gcpan_run->{cpan}{lc($original_ebuild)}{depends}})
            {
                ...
                # Remove dep from list of modules to install later on - no more dup'd installs!
            ...
            foreach my $target_dir (@overlays)
            {
                if (-d $target_dir)
                {
                    ...
                    # Break out if we already have an ebuild (upgrade or mistake in the code)
                    if (!-f $ebuild)
                    {
...
HERE
                        ...
                        if (my @deps = keys %{$gcpan_run->{cpan}{lc($original_ebuild)}{depends}})
                        {
                            ...
                            foreach my $dep (@deps)
                            {
                                ...
                                # Last ditch call to scanTree to make sure we have info
...
When you run B<g-cpan>, it will check for two configuration files. If you are root, it will check for the presence of an already configured CPAN under your perl install path. If CPAN is not configured, or you are not root, g-cpan will create a generic configuration for CPAN in C<~/.cpan/CPAN/> called C<MyConfig.pm>. You can modify this file as needed at any time.
...
B<g-cpan> is now overlay "friendly." B<g-cpan> will scan both the overlays provided in your C<make.conf> as well as any you have set via environment variables, to help determine its course of action.
If you have defined overlays, B<g-cpan> will use the first overlay in your list that the user running it can write to.
Any ebuilds generated by B<g-cpan> will be stored in this overlay for future use (such as upgrading).
...
If you want to generate ebuilds to a specific overlay, you may set GCPAN_OVERLAY in your C<make.conf>, C<.gcpanrc> or environment.

If you want to generate ebuilds into a specific category, you may set GCPAN_CAT in your C<make.conf>, C<.gcpanrc> or environment.
...
Log all output to C</var/log/g-cpan/g-cpan.log> and C<g-cpan.err>
...
=head2 Upgrading specific ebuild in overlay from g-cpan
...
=for stopwords mcummings beu RAC Vermeulen Krotkine Yuval Yaari

mcummings <mcummings@gentoo.org>, cab <cab@gentoo.org>, sniper <sniper@mongueurs.net>, beu <beu@gentoo.org>,
J Robert Ray <jrray@gentoo.org> (g-cpan), RAC <rac@gentoo.org>, Sven Vermeulen <swift@gentoo.org> (Original manpage),
Damien Krotkine <dams@gentoo.org>, Yuval Yaari <yuval@gentoo.org>, Robin J Johnson <robbat2@gentoo.org>.
